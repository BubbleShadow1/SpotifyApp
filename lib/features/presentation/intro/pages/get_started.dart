...
import 'package:spotify/features/presentation/choose_mode/pages/choose_mode.dart';
...
class GetStartedState extends State<GetStarted>{
  ...
  Widget build(BuildContext context) {
   return Scaffold(body: Stack(fit:StackFit.expand,children: [
     Container(padding:EdgeInsets.only(left: 20,right: 20) ,child:Column(children: [
      ...
      Container(padding:EdgeInsets.only(left: 10,right: 10,top: 30,bottom: 20),child: Text('Listening Music is an Art that does not a cup of Tea of Everyone so listen music EveryTime and Enjoy Moment ',style:TextStyle(color: appcolors.greycolor,fontSize:MediaQuery.of(context).size.height/50),),),
       BasicAppButton('Get Started',(){Navigator.pushReplacement(context, MaterialPageRoute(builder: (context)=>ChooseMode())); } , 80),

     ],),decoration:BoxDecoration(image: DecorationImage(fit: BoxFit.fill,image: AssetImage(appimages.splashscreenBG),),) ,),
